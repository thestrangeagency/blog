// generated on 2016-01-03 using generator-gulp-webapp 1.0.3
...
gulp.task('jekyll:prod', ['useref'], done => {
...
gulp.task('useref', ['styles', 'copy-jekyll-posts'], () => {
...
gulp.task('copy-jekyll-posts', () => {
  return gulp.src(['app/_posts/*', 'app/_templates/*'], {
    base: 'app'
  }).pipe(gulp.dest('.tmp'));
});
...
gulp.task('bower-images', () => {
  return gulp.src('bower_components/**/*.gif')
    .pipe($.flatten())
...
gulp.task('serve', ['jekyll', 'styles', 'fonts'], () => {
  browserSync({
    notify: false,
    port: 9000,
    server: {
